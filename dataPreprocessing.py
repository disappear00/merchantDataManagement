...
def main():
    print("请选择第一个包含商家ID的文件...")
    file1_path = select_file("选择第一个文件")
    if not file1_path:
        print("未选择文件，程序退出")
        return
    
    print("请选择第二个包含商家ID和外卖组织结构的文件...")
    file2_path = select_file("选择第二个文件")
    if not file2_path:
        print("未选择文件，程序退出")
        return
    
    print("请选择第三个包含商家ID和一系列数据的文件...")
    ...
    print(f"已选择输出文件夹：{output_folder}")
    ...
    try:
        ...
        print(f"将更改 {rows_to_update} 行数据的外卖组织结构为'霸州三组'")
        ...
        for org_name, group_data in df3.groupby(org_structure_col):
            ...
            print(f"已保存 {len(group_data)} 行数据到 {output_name}")
            total_processed += len(group_data)
        
        # 验证所有数据都被处理
        print(f"\n总共处理了 {total_processed} 行数据，原始文件3有 {len(df3)} 行数据")
        if total_processed != len(df3):
            print(f"警告：处理的数据行数与原始文件不一致，差异为 {len(df3) - total_processed} 行")
        else:
            print("验证成功：所有数据都已正确处理并保存到各分组文件中")
        ...
        print(f"已保存更新后的文件3到 {updated_file3_path}")
        ...
        if missing_org_count > 0:
            ...
            print(f"已保存缺失组织结构的商家ID列表到 {missing_ids_path}")
